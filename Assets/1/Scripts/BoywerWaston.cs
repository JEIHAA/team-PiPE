...
public class BoywerWaston 
{
    ...
    public static BoywerWaston Triangulate(List<Point> points)
    {
        BoywerWaston delaunay = new BoywerWaston();
        delaunay.pointList = points;
    ...
    private void setTetrahedron()
    {
        ...
        foreach (var point in pointList) // ex)포인트 11개를 돌때까지 검사
        {
            ...
            foreach (Tetrahedron tri in triangulation)// 리스트내의 삼각형 검사
            {
                ...
                {
                    Debug.Log(tri.A.Position + " " + tri.B.Position + " " + tri.C.Position + " " + tri.D.Position + "TRIANGLE");
                    Debug.Log(tri.Circumscriber.Center + " " + tri.Circumscriber.Radius + " " + Vector3.Distance(tri.Circumscriber.Center, point.Position) + " sphere");
